...
def experiment_five(data_path, output_path, dataset_name="MNIST", n_neighbors=80):
    ...
    for i in range(5):
        ...
        start_time = time()
        X_low = LargeVis.run(
            2, 16, -1, -1, -1, -1, -1, 3 * n_neighbors, -1, n_neighbors
    ...
    np.save(
        output_path
        + "/{dataset_name}_{method}_{n_neighbors}".format(
            dataset_name=dataset_name, method=method, n_neighbors=n_neighbors
        ),
        X_lows,
    )
    print(
        "Total time for method {method} on {dataset_name} with {n_neighbors} is {avg_time}".format(
            method=method,
...
def main():
    ...
    method = "LargeVis"
    np.save(
        "/home/home1/hh219/PaCMAP/output_{dataset_name}_{method}_{n_neighbors}".format(
            dataset_name=dataset_name, method=method, n_neighbors=n_neighbors
        ),
        X_low,
    )
    print(
        "Total time for method {method} on {dataset_name} is {total_time}".format(
            method=method, dataset_name=dataset_name, total_time=total_time
        )
    )
