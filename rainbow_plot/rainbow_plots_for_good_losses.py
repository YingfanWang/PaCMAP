...
# TODO: check the input embedding


X, labels = data_prep(dataset='MNIST')
y_ = np.zeros(labels.shape[0])
for i in range(labels.shape[0]):
    y_[i] = np.argmax(labels[i]) % 10
...
# parameter "a" and "b" use default value as below
def attr(x):
...
plt.axes([0.04, 0.74, 0.21, 0.23])
plt.title('t-SNE', fontsize=38)
Y_norm = np.load('t-SNE_MNIST_20.npy')
plt.scatter(Y_norm[:, 0], Y_norm[:, 1], c=y_, s=0.03, cmap=cmap_fig)
plt.xticks([])
plt.yticks([])
plt.axis('off')

plt.axes([0.047, 0.39, 0.2, 0.29])
...
plt.axes([0.29, 0.74, 0.21, 0.23])
plt.title('UMAP', fontsize=38)
reducer = umap.umap_.UMAP(n_components=2)
embedding = reducer.fit_transform(X)
y_min, y_max = embedding.min(0), embedding.max(0)
Y_norm = (embedding - y_min) / (y_max - y_min)  # 归一化
...
plt.axes([0.54, 0.74, 0.21, 0.23])
plt.title('TriMAP', fontsize=38)
Y = np.load('trimap_MNIST_10.npy')
plt.scatter(Y[:, 0], Y[:, 1], c=y_, s=0.03, cmap=cmap_fig)
plt.xticks([])
plt.yticks([])
plt.axis('off')

plt.axes([0.543, 0.39, 0.2, 0.29])
...
plt.axes([0.79, 0.74, 0.21, 0.23])
plt.title('PaCMAP', fontsize=38)
Y = np.load('MNIST_0_5__.npy')
