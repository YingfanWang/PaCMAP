...
def test_pacmap_randomness_deterministic():
    ...
    instance1 = pacmap.PaCMAP(
        n_components=2, n_neighbors=10, lr=1, random_state=20, apply_pca=True
    )
    instance1_out = instance1.fit_transform(sample_data, init="pca")
    instance2 = pacmap.PaCMAP(
        n_components=2, n_neighbors=10, lr=1, random_state=20, apply_pca=True
    )
    ...
    try:
        ...
        try:
            assert (
                np.sum(
                    np.abs(
                        instance1.pair_FP.astype(int) - instance2.pair_FP.astype(int)
                    )
                )
                < 1e-8
            )
            assert (
                np.sum(
                    np.abs(
                        instance1.pair_MN.astype(int) - instance2.pair_MN.astype(int)
                    )
                )
                < 1e-8
            )
